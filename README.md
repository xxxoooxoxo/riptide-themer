...
An unofficial macOS style patch for `/Applications/HumanLayer.app` (`com.humanlayer.riptide`). It injects [`rounded.css`](./rounded.css) and a selected theme preset into the app's `WKWebView`, then reapplies the patch after login and HumanLayer updates.

## Preview

![Let's Get Nauti theme applied to HumanLayer](./assets/lets-get-nauti-preview.png)

## How it works

1. `wk-css-injector.m` builds an ad-hoc-signed dynamic library that installs the base stylesheet and selected file from [`presets/`](./presets) at document start.
2. `patch-riptide-beta.sh` adds the injector, stylesheet, and persisted preset selection to HumanLayer's `LSEnvironment`, then re-signs the app with library validation disabled.
...
- `vercel-dark` — neutral black surfaces with restrained blue and status accents
- `lets-get-nauti` — warm Pocket-inspired layers with Nautilus Blue (`#06B2DD`) for primary actions and focus

Select a preset and reopen HumanLayer:

```sh
./set-theme.sh lets-get-nauti
```

The selection persists through login patches and HumanLayer updates. See the [theming guide](./THEMING.md) to create another preset.

## Change styles

Edit `rounded.css` for shared styling or the active file in `presets/`, then quit and reopen HumanLayer. CSS-only changes do not require rebuilding or re-signing the app.
...
The editor uses the captured HumanLayer task page as its preview canvas. It loads the same `rounded.css` and `presets/*.css` files as the native injector, provides an exact Original/Rounded A/B view, and includes live palette, radius, font, viewport, and custom CSS controls. Pocket's component-only dark code and toast tokens are not presented as a full dark preset.
